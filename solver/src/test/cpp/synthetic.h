#ifndef HUSK_TEST_SYNTHETIC_H_
#define HUSK_TEST_SYNTHETIC_H_
...
inline std::array<double, 9> Identity3() { return {1, 0, 0, 0, 1, 0, 0, 0, 1}; }

inline std::array<double, 9> RotationAboutY(double radians) {
...
inline std::array<double, 16> DualQuadricOf(const TestEllipsoid& e) {
...
inline std::array<double, 3> Normalise(std::array<double, 3> v) {
...
inline CameraView LookAt(const std::array<double, 3>& eye, const std::array<double, 3>& target) {
...
inline DualConic Project(const CameraView& view, const std::array<double, 16>& quadric) {
...
inline std::vector<CameraView> ViewsAround(const std::array<double, 3>& target, double distance,
...
inline std::vector<CameraView> WellSeparatedViews(const std::array<double, 3>& target, double distance) {
  return ViewsAround(target, distance, {0.0, 1.1, 2.3, 3.6, 4.9}, 0.25);
}

inline std::vector<DualConic> ProjectAll(const std::vector<CameraView>& views,
...
inline Ellipse EllipseOf(const DualConic& c) {
...
inline DualConic ConicOf(const Ellipse& e) {
...
inline std::vector<DualConic> WithTracingError(std::vector<DualConic> conics) {
  int step = 0;
